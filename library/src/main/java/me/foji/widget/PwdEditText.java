 ...
 * 支付密码输入框
 *
 * @author Scott Smith @Date 2016年08月16/8/27日 22:59
 */
public class PwdEditText extends EditText {
    // 默认外边缘线条宽度为1dp
    private final float DEFAULT_OUT_LINE_WIDTH = DensityUtilKt.dp2px(getContext(), 1.0f);
    // 默认外边缘线条颜色
    private final int DEFAULT_OUT_LINE_COLOR = Color.parseColor("#bebebe");
    // 默认外边缘弧度
    private final float DEFAULT_OUT_RADIUS = 0f;
    // 默认间隔线宽度
    private final float DEFAULT_DIVIDER_LINE_WIDTH = DensityUtilKt.dp2px(getContext(),1.0f);
    // 默认间隔线颜色
    private final int DEFAULT_DIVIDER_LINE_COLOR = Color.parseColor("#eeeeee");
    // 默认占位圆点半径
    private final float DEFAULT_DOT_RADIUS = DensityUtilKt.dp2px(getContext(),7.5f);
    // 默认占位圆点颜色
    private final int DEFAULT_DOT_COLOR = Color.BLACK;
    // 默认输入框宽度
    private final float DEFAULT_INPUT_BOX_WIDTH = DensityUtilKt.dp2px(getContext(),47f);
    // 默认密码长度
